...
export function ShopProvider({ children }) {
  ...
  function handleSearch(event) {
    ...
    if (shopCategory) {
      _products = _products.filter(
        (_item) => _item.category === shopCategory.toLowerCase()
      );
    }
  ...
  function handleAttrFilters(event) {
    ...
    if (
      ...
      event.target.classList.contains("shop-category")
    ) {
      filterExecuted = 1;
    ...
    if (filterExecuted) {
      setProducts(_products ?? []);
      setShopCategory(_toyCategory ?? shopCategory);
